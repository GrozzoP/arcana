...
### Espacial
El diccionario `memo` almacena como máximo $(m + 1) \times (n + 1)$ entradas. A esto se suma el espacio de la [pila](stack.md) de recursión, que en el peor caso tiene profundidad $O(m + n)$ cuando no hay `*` y se avanza un índice por llamada. 
...
### Limitaciones
- La recursión implica overhead de llamadas a función y uso de [pila](stack.md). En lenguajes sin optimización de tail-call (como Python), instancias muy grandes pueden provocar `RecursionError`.
